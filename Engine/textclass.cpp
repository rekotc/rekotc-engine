						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = InitializeSentence(&m_sentence1, MAX_STRING_LENGTH, device);
	if(!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence1, "Running time: ", 20, 20, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence2, MAX_STRING_LENGTH, device);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Number of objects: ", 20, 40, 1.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::updateText(int textID, float parameter, ID3D11DeviceContext* deviceContext)
{
	char tempString[32];
	char countString[32];
	int posX, posY;
	...
	switch (textID){
	//textID = 0 identifica il timer
	case 0:
		strcpy_s(countString, "Running time: ");
		sprintf(tempString, "%f", parameter);
		sentence = m_sentence1;
		posX = 20;
		posY = 20;
		break;
	case 1:
	//textID = 1 identifica il rendercount
		strcpy_s(countString, "Number of objects: ");
		sprintf(tempString, "%f", parameter);
	...
	//_itoa_s((int)runningTime, tempString, 10);
	
	strcat_s(countString, tempString);
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(sentence, countString, posX, posY, 1.0f, 1.0f, 1.0f, deviceContext);
